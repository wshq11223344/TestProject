...
//        setTranslucentStatus(activity);
        if (fontIconDark) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                setStatusBarFontIconDark(activity1, TYPE_M);
            } else if (OSUtils.isMiui()) {
                setStatusBarFontIconDark(activity1, TYPE_MIUI);
            } else if (OSUtils.isFlyme()) {
                setStatusBarFontIconDark(activity1, TYPE_FLYME);
            } else {//其他情况下我们将状态栏设置为灰色，就不会看不见字体
                setStatusBarColor(activity1, Color.LTGRAY);//灰色
            }
        }
    }
